...
class Gacha(commands.Cog):
    ...
    def _wrong_channel(self, interaction: discord.Interaction, cmd_key: str, config: dict) -> str | None:
        limits = config.get("channel_limits", {})
        ch_id = limits.get(cmd_key, "")
    ...
    async def checkin(self, interaction: discord.Interaction):
        ...
        if err := self._wrong_channel(interaction, "checkin", config):
            return await interaction.response.send_message(err, ephemeral=True)
        await interaction.response.defer()
        success, msg = await asyncio.to_thread(db.checkin, gid, str(interaction.user.id), config)
        ...
        if success:
            ...
            embed.add_field(name=f"目前{_label(config)}", value=f"🪙 {user[_col(config)]}")
    ...
    async def work(self, interaction: discord.Interaction):
        ...
        work_list = config["work"]
        label = _label(config)
        if len(work_list) == 1:
            success, msg = await asyncio.to_thread(db.start_work, gid, str(interaction.user.id), work_list[0], config)
            color = 0x3498DB if success else 0xE74C3C
            embed = discord.Embed(title="💼 打工", description=msg, color=color)
            await interaction.followup.send(embed=embed)
            return

        view = WorkSelectView(config, interaction.user.id, gid)
        embed = discord.Embed(
            title="💼 選擇打工地點",
            description="\n".join(
    ...
    async def collect(self, interaction: discord.Interaction):
        ...
        gid = self._gid(interaction)
        await interaction.response.defer(ephemeral=True)
        success, msg = await asyncio.to_thread(db.collect_work, gid, str(interaction.user.id), config)
        color = 0x2ECC71 if success else 0xE74C3C
    ...
    async def gacha(self, interaction: discord.Interaction):
        config = self._cfg(interaction)
        gid = self._gid(interaction)
        if err := self._wrong_channel(interaction, "gacha", config):
            return await interaction.response.send_message(err, ephemeral=True)
        await interaction.response.defer()
        success, msg, prize = await asyncio.to_thread(db.do_gacha, gid, str(interaction.user.id), config)
        if not success:
    ...
    async def status(self, interaction: discord.Interaction):
        ...
        gid = self._gid(interaction)
        await interaction.response.defer(ephemeral=True)
        status = await asyncio.to_thread(db.get_status, gid, str(interaction.user.id), config)
    ...
    async def pool(self, interaction: discord.Interaction):
        config = self._cfg(interaction)
        await interaction.response.defer()
        items = db.calc_item_probabilities(config)
    ...
    async def adventure(self, interaction: discord.Interaction):
        config = self._cfg(interaction)
        gid = self._gid(interaction)
        if err := self._wrong_channel(interaction, "adventure", config):
            return await interaction.response.send_message(err, ephemeral=True)
        await interaction.response.defer()
        ...
        label = _label(config)
        view = AdventureSelectView(config, interaction.user.id, gid)
        lines = []
        for a in adventures:
        ...
        embed = discord.Embed(title="⚔️ 選擇冒險", description="\n\n".join(lines), color=0xE67E22)
        await interaction.followup.send(embed=embed, view=view)
    ...
    async def redeem(self, interaction: discord.Interaction):
        config = self._cfg(interaction)
        gid = self._gid(interaction)
        if err := self._wrong_channel(interaction, "redeem_cmd", config):
            return await interaction.response.send_message(err, ephemeral=True)
        await interaction.response.defer()
        items = await asyncio.to_thread(db.get_inventory, gid, str(interaction.user.id))
        if not items:
            embed = discord.Embed(title="🎁 兌換", description="背包是空的，沒有可兌換的獎品！", color=0x95A5A6)
            await interaction.followup.send(embed=embed)
            return

        view = RedeemSelectView(config, self.bot, interaction.user, interaction.channel, items, gid)
        embed = discord.Embed(
            title="🎁 選擇要兌換的獎品",
...
class WorkSelectView(discord.ui.View):
    def __init__(self, config: dict, user_id: int, guild_id: str):
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        success, msg = await asyncio.to_thread(db.start_work, self.guild_id, self.user_id, work_cfg, self.config)
        color = 0x3498DB if success else 0xE74C3C
        embed = discord.Embed(title="💼 打工", description=msg, color=color)
...
class AdventureSelectView(discord.ui.View):
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        if adventure["cost_type"] == "custom_tokens":
            modal = CustomBetModal(self.config, self.user_id, adventure, self.guild_id)
    ...
    async def _run_adventure(self, interaction, adventure, custom_amount=None):
        ...
        result = await asyncio.to_thread(db.do_adventure, self.guild_id, self.user_id, adv, self.config)
        ...
        user = await asyncio.to_thread(db.get_user, self.guild_id, self.user_id, self.config)
...
class CustomBetModal(discord.ui.Modal):
    def __init__(self, config: dict, user_id: str, adventure: dict, guild_id: str):
        label = _label(config)
...
class RedeemSelectView(discord.ui.View):
    def __init__(self, config: dict, bot, user: discord.Member, channel, items: list[dict], guild_id: str):
        super().__init__(timeout=60)
        self.config = config
        self.bot = bot
        self.user = user
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        idx = int(interaction.data["values"][0])
        item = self.items[idx]
        view = RedeemConfirmView(self.config, self.bot, self.user, self.channel, item, self.guild_id)
        embed = discord.Embed(
            title="🎁 確認兌換",
...
class RedeemConfirmView(discord.ui.View):
    def __init__(self, config: dict, bot, user: discord.Member, channel, item: dict, guild_id: str):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        public_embed = discord.Embed(title="🎁 獎品兌換通知", description=msg, color=0xE91E63)
